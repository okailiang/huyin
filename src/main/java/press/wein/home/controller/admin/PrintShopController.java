...
public class PrintShopController extends BaseController {
    ...
    @ResponseBody
    private ResponseEntity<Object> getPrintShop(@RequestParam(value = "id") Long id) throws ServiceException {
        PrintShopVo printShopVo = printShopService.getPrintShop(id);
        printShopVo.setPrintShopImage(FileUtil.imageToBase64(printShopVo.getPrintShopImage()));
        return ResponseUtils.success(printShopVo);
    }
    ...
    @ResponseBody
    public String savePrinterInfo(HttpServletRequest request,
                                  HttpServletResponse response,
                                  @RequestParam("shopImageFile") MultipartFile shopImage) throws ServiceException {
        String shopImageName = shopImage.getOriginalFilename();
        long shopIamgeSize = shopImage.getSize();

        // 大于3M图片不允许上传
        if (shopIamgeSize > (2 * (long) 1073741824)) {
        ...
        try {
            ...
            byte[] imageByte = new byte[shopImage.getInputStream().available()];
            shopImage.getInputStream().read(imageByte);
            String base = Base64.getEncoder().encodeToString(imageByte);

            System.out.println(base);
            System.out.println(MD5Util.md5Hex(imageByte));
            System.out.println(MD5Util.md5Hex(shopImage.getBytes()));
            ...
            // 将图片拷贝到服务器
            shopImage.transferTo(outFile);
        } catch (IOException e) {
            e.printStackTrace();
        }

        return TipConstants.DISABLE_SUCCESS;
    }

    @RequestMapping(value = "/getShopImage", method = RequestMethod.GET)
    @ResponseBody
    public String getShopImage(HttpServletRequest request,
                               HttpServletResponse response) {
