	...
	$.ajax({
		...
		success : function(data) {
			if (data.success && (data.result.orderState == 2)) {
				//window.location.href = "jsp/users/doneOrder.jsp?orderNo="
					//	+ orderNo;
				paySuccessTip("支付成功");
...
//					window.location.href = "jsp/users/order.jsp";
//				} else if (data.result.users.role == 4) {
//					window.location.href = "jsp/users/printFile.jsp";
//				}
...
var showOrders = function(data) {
	...
	for ( var i = 0; i < orderArrLen; i++) {
		...
		if(!(orderArray[i].orderState==1&&orderArray[i].directPrint==1)){
			...
			html += '	<span class="qrcode-thumbnail btn btn-sm btn-default" id="qrcode-thumbnail-'
				+orderNo
	...
	$(".qrcode-thumbnail").mouseover(function() {
	...
	$(".qrcode-thumbnail").mouseout(function() {
		var orderNo = $(this).attr("id").split("-")[2];
		$("#qrcode-"+orderNo).addClass("hidden");
	});
	$(".qrcode-thumbnail").click(function() {
		var orderNo = $(this).attr("id").split("-")[2];
