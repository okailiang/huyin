...
var ZYFILE = {
		...
		funGetFiles : function(e){
			...
			var lArr = [];  // 之前文件的名称数组
			var uArr = [];  // 现在文件的名称数组
			$.each(this.lastUploadFile, function(k, v){
				lArr.push(v.name);
			});
			...
			$.each(uArr, function(k, v){
				...
				if($.inArray(v, lArr) < 0){  // 不存在
					...
					self.lastUploadFile = self.lastUploadFile.concat(self.uploadFile);
				}else{
					alert("你已经上传过该文件！");
				}
			});
