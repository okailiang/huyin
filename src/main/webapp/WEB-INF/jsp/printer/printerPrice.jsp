...
<div id="pop-div" class="register-div">
	...
	<div id="pop-div" class="register-div-descript">
	...
	<form class="form-horizontal" role="form" method="post">
		<div class="form-group">
			...
			<div class="col-md-9">
				<select class="form-control" name="file-type" id="filetype-choice"
					autocomplete="off">
		...
		<div class="form-group">
			...
			<div class="col-md-9">
				<select class="form-control" name="price-type" autocomplete="off"
					id="pricetype-choice">
		...
		<div class="form-group">
			...
			<div class="col-md-9">
				<input type="text" class="form-control" id="price" name="price"
					placeholder="小数点后两位，如 0.10" autocomplete="off">
			</div>
		</div>
		<div class="form-group">
			<div class="col-md-8">
				<button type="button" name="button"
					class="btn btn-primary pull-right" id="printer-add-price">
...
<div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 content">
	...
	<div class="order-btn-group">
		...
		<button id="printer-del-multi-price" class="btn btn-danger">
			<span class="glyphicon glyphicon-trash"></span>&nbsp;&nbsp;删除
		</button>
		<button id="add-price" class="btn btn-info" onclick="showDiv()">
			<span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;添加
		</button>
		<%-- <form action="searchPrintPrice.do">
		    <input
				type="text" name="username" class="form-control" placeholder="请输入文件类型">
				...
				<u:choose>
					<u:when test="${!empty pricesList}">
						<u:forEach items="${pricesList}" var="item">
							<tr id="tr${item.id}">
								<td id="tr${item.id}td1"><input type="checkbox"
									value="${item.id}" />
								</td>
								<td>${(item.fileType eq 1) ? "doc/docx" :((item.fileType eq
									2)?"ppt/pptx":((item.fileType eq 3)?"pdf":"png/jpg/jpeg"))}</td>
								<td>${item.priceType}</td>
								<td>${item.price}</td>
								<td><a href="javascript:void(0)" id="print-file" class="print-file"
									value="${item.id}" onclick="printerEditPrice('${item.id}')" Id=${item.id}"><span
										class="glyphicon glyphicon-edit" title="编辑"></span>
								</a> <a href="javascript:void(0)"
									onclick="removePrice('${item.id}')" id="${item.id}"><span
										class="glyphicon glyphicon-trash error" title="删除"></span>
								</a></td>
							</tr>
						</u:forEach>
					</u:when>
